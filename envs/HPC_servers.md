...
* Ensure that your SSH key is properly configured to connect to OSC.
    * If you don’t have an SSH key yet, refer to this guide:  [Generating a new SSH key and adding it to the ssh-agent](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent).
 * Modify your VSCode Remote-SSH config (~/.ssh/config) accordingly.
    * Set `Host` to your favoriate, and set `IdentityFile` with the real SSH key path. you need to replace **user_name** with your **actual OSC username**.
    * `HostName` can't be renamed, should be server name.
...
# Submit job through Slurm on OSC
Slurm job template:
* Send job condition via email
* Replace account number
``` bash
#!/usr/bin/bash
#SBATCH --job-name OSC_job
...
source ~/.bashrc
# conda activate env # (optional) activate conda environment
```
